...
function xmldb_local_stream_upgrade($oldversion) {
    ...
    // Force-add missing Zoom deletion fields (for environments where plugin version advanced but DB did not).
    ...
    if ($oldversion < 2026051707) {
        $table = new xmldb_table('local_stream_qgen_log');
        ...
        $table->add_field('videotitle', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table->add_field('qtype', XMLDB_TYPE_CHAR, '32', null, XMLDB_NOTNULL, null, null);
        $table->add_field('questioncount', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        ...
        $table->add_index('qtype', XMLDB_INDEX_NOTUNIQUE, ['qtype']);

        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        upgrade_plugin_savepoint(true, 2026051707, 'local', 'stream');
    }
