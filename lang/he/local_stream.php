<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * local_stream lang:he
 *
 * @package    local_stream
 * @copyright  2023 mattandor <mattan@centricapp.co.il.il>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

$string['above'] = 'מעל הפעילות';
$string['accountid'] = 'מזהה חשבון';
$string['adddate'] = 'תאריך לשם הפעילות';
$string['addrecordingtype'] = 'הוספת סוג הקלטה';
$string['addrecordingtype_desc'] = 'כאשר מופעל, סוג ההקלטה יופיע בשם הפעילות.';
$string['areyousure'] = 'האם אתה בטוח?';
$string['audio_quality'] = 'איכות אודיו';
$string['basedgrouping'] = 'שיבוץ הקלטה בודדת';
$string['basedgrouping_desc'] = 'כאשר מופעל, שיבוץ הפעילות בתוך הקורס יתאפשר רק עבור סוג הקלטה אחד בלבד.';
$string['below'] = 'מתחת לפעילות';
$string['browselist'] = 'לוח ההקלטות';
$string['clientid'] = 'מזהה לקוח';
$string['clientsecret'] = 'סוד לקוח';
$string['course'] = 'שם הקורס';
$string['coursedashboard'] = 'ההקלטות שלי';
$string['dashboard'] = 'לוח ההקלטות';
$string['daystocleanup'] = 'ניקוי מקור ישן יותר מ';
$string['daystocleanup_desc'] = 'שימו לב: הפעלת תכונה זו היא באחריותו הבלעדית של הלקוח.';
$string['daystolisting'] = 'ימים לסנכרון';
$string['deleterecordings'] = 'מחיקת הקלטות';
$string['department'] = 'מחלקה';
$string['downloadrecordings'] = 'הורדת הקלטות';
$string['duration'] = 'אורך';
$string['email'] = 'דוא״ל';
$string['embeddedrecordings'] = 'שיבוץ הקלטות';
$string['embeddingsettings'] = 'שיבוץ';
$string['endtime'] = 'שעת סיום';
$string['error'] = 'אירעה שגיאה במערכת, אנא דווח על כך למנהל.';
$string['errordownload'] = 'לא ניתן להוריד את ההקלטה #{$a}.';
$string['externalip'] = 'כתובת URL שרת אחסון';
$string['externalpass'] = 'סיסמא';
$string['externaluser'] = 'שם משתמש';
$string['extrafields'] = 'שדות נוספים';
$string['file_extension'] = 'סיומת קובץ';
$string['file_size'] = 'גודל הקובץ';
$string['filters'] = 'מסננים';
$string['has_screen_share'] = 'יש שיתוף מסך';
$string['hidetopic'] = 'הסתרת נושא';
$string['host'] = 'מארח';
$string['indexrecordings'] = 'יומן הקלטות';
$string['invalid'] = 'שגוי';
$string['meeting'] = 'פגישה';
$string['meetingid'] = 'מזהה פגישה';
$string['messagenewvideocontent'] =
        'שמחים לבשר לך שההקלטה החדשה שלך, בנושא <b>{$a->topic}</b>,  מתאריך <u>{$a->date}</u> בשעה <u>{$a->time}</u>, זמינה לצפייה כעת בלוח ההקלטות. {$a->videolink}';
$string['messagenewvideosubject'] = 'הקלטה חדשה: {$a}';
$string['viewvideo'] = 'צפה בסרטון';
$string['messageprovider:uploaded'] = 'הודעה על סרטון חדש שהועלה';
$string['nodownload'] = 'ללא סנכרון';
$string['order_desc'] = 'שיבוץ של ההקלטה מעל/מתחת למפגש המקורי.';
$string['participants'] = 'משתתפים';
$string['participantsviews'] = 'צפיות/משת׳';
$string['platform'] = 'פלטפורמה לשיחות ועידת וידאו';
$string['platform_desc'] = 'יש לבחור את הפלטפורמה לשיחות ועידת וידאו המתאים.';
$string['platform_settings'] = 'הגדרות פלטפורמה לשיחות ועידת וידאו';
$string['pluginname'] = 'Stream';
$string['prefix'] = 'קידומת לשם הפעילות';
$string['prefix_desc'] = 'קידומת זו תופיע בתחילת שם ההקלטה בקורס.';
$string['privacy:metadata:local_stream_rec'] = 'נתונים אישיים מטבלת הפגישות.';
$string['privacy:metadata:local_stream_rec:email'] = 'נתוני דוא"ל משתמש מטבלת הפגישות.';
$string['privacy:metadata:stream'] =
        'על מנת להשתלב עם שירות Stream מרוחק, יש להחליף נתוני משתמש עם השירות הזה';
$string['privacy:metadata:stream:email'] = 'האימייל נשלח ממודל כדי לאפשר לך לגשת לנתונים שלך בשרת Stream.';
$string['privacy:metadata:stream:fullname'] = 'שמך המלא נשלח למערכת המרוחקת כדי לאפשר חווית משתמש טובה יותר';
$string['progress'] = 'בתהליך';
$string['recording'] = 'הקלטה';
$string['recording_type'] = 'סוג הקלטה';
$string['recordingcycle'] = 'השינויים שבוצעו בהקלטה #{$a} נשמרו בהצלחה.';
$string['recordingdeleted'] = 'ההקלטה #{$a} נמחקה בהצלחה.';
$string['recordings'] = 'הקלטות';
$string['recordingshidden'] = 'ההקלטה #{$a} הוסתרה בקורס.';
$string['recordingshow'] = 'ההקלטה #{$a} מוצגת באופן גלוי בקורס.';
$string['recordingsperpage'] = 'מספר הקלטות לעמוד';
$string['refreshtoken'] = 'יצירת אסימון חדש';
$string['secrettoken'] = 'מזהה טוקן Webhook';
$string['settingspage'] = 'הגדרות Stream';
$string['starttime'] = 'שעת התחלה';
$string['status'] = 'סטטוס';
$string['storage'] = 'סנכרון הקלטות';
$string['stream:edit'] = 'עריכת נתוני הקלטה';
$string['stream:manage'] = 'ניהול וצפיה בהקלטות';
$string['stream:view'] = 'צפיה בהקלטות זום';
$string['streamcategoryid'] = 'ערוץ ברירת מחדל ב-Stream';
$string['streamdashboard'] = 'גישה למערכת Stream';
$string['streamkey'] = 'מפתח Stream';
$string['streamkey_desc'] = 'כדי לקבל את המפתח, אנא <a href="https://stream-platform.cloud/" target="_blank">בקר כאן</a>.';
$string['streamurl'] = 'כתובת URL מערכת Stream';
$string['syncmteams'] = 'Sync MS Teams';
$string['teams'] = 'מיקרוסופט טימס';
$string['teamsusersfilter'] = 'סינון שמות משתמשים';
$string['teamssharepointsite'] = 'אתר SharePoint (ספריית הקלטות טימס)';
$string['teamssharepointsite_desc'] =
        'חובה לאינדוקס טימס (אלא אם מסומן "Legacy" למטה). מזהה אתר ב-Microsoft Graph לאתר SharePoint שבו נשמרות הקלטות — לדוגמה contoso.sharepoint.com:/sites/RecordingsSite:. ' .
        'כשהשדה מלא נסרקים כל הכוננים באתר וכל התיקיות לקובצי וידאו. נדרשות הרשאות אפליקציה כגון Sites.Read.All ו-Files.Read.All.';
$string['teamslegacyonedrive'] = 'אינדוקס ישן (קבוצות + OneDrive)';
$string['teamslegacyonedrive_desc'] =
        'רק אם לא משתמשים בשדה אתר SharePoint למעלה: מסלול איטי שמביא קבוצות מ-Entra, משאיר רק קבוצות שבעלים בהן עם "moodle@" בדוא״ל, ואז סורק OneDrive שלהם. השאירו לא מסומן כשמשתמשים באתר SharePoint למעלה.';
$string['teamsfallbackemail'] = 'דוא״ל fallback להעלאה ל-Stream';
$string['teamsfallbackemail_desc'] =
        'אופציונלי. כש-Graph לא מחזיר דוא״ל יוצר לקובץ ב-SharePoint, משתמשים בכתובת זו כבעלים ב-Stream. אם ריק — יילקח דוא״ל מנהל האתר.';
$string['tenantid'] = 'Tenant ID';
$string['topic'] = 'נושא';
$string['unicko'] = 'יוניקו';
$string['uploadmedia'] = 'העלאת מדיה חדשה';
$string['user_type'] = 'סוג משתמש';
$string['uuid'] = 'UUID';
$string['valid'] = 'תקין';
$string['video_quality'] = 'איכות וידאו';
$string['views'] = 'צפיות';
$string['webex'] = 'וובקס';
$string['webexjwt'] = 'מזהה JWT';
$string['zoom'] = 'זום';
$string['defaultcollectionmode'] = 'מצב אוסף ברירת מחדל';
$string['defaultcollectionmode_desc'] = 'כאשר מופעל, המופע הראשון של mod_stream שנוצר בקורס יהיה עם collection_mode מוגדר כ-true, מה שמאפשר לקבץ מספר סרטונים יחד.';
$string['hours'] = 'שעות';
$string['never'] = 'לעולם לא';
$string['studentaccessdisabled'] = 'הגישה של תלמידים ללוח ההקלטות הושבתה על ידי המנהל.';
$string['zoom_delete_after_hours'] = 'מחיקת הקלטת Zoom לאחר שיבוץ';
$string['zoom_delete_after_hours_desc'] = 'רק עבור Zoom: מחק את ההקלטה מענן Zoom מספר שעות לאחר שאונדקסה ושובצה בקורס. חוסך אחסון ב-Zoom; הצפייה במודל ממשיכה מההעתק המשובץ אם משתמשים באחסון Stream.';
$string['zoom_account_stats'] = 'סטטיסטיקות חשבון Zoom';
$string['zoom_account_stats_heading_desc'] = 'צפייה בסטטיסטיקות החשבון (משתמשים, עם רישיון לעומת ללא רישיון, נפח אחסון). <a href="{$a}">פתח סטטיסטיקות חשבון Zoom</a>';
$string['zoom_stats_zoom_only'] = 'דף זה זמין רק כאשר הפלטפורמה מוגדרת ל-Zoom.';
$string['zoom_stats_error'] = 'לא ניתן לטעון את סטטיסטיקות Zoom:';
$string['zoom_stats_metric'] = 'מדד';
$string['zoom_stats_value'] = 'ערך';
$string['zoom_stats_total_users'] = 'סך המשתמשים';
$string['zoom_stats_licensed_users'] = 'משתמשים עם רישיון';
$string['zoom_stats_basic_users'] = 'משתמשים ללא רישיון';
$string['zoom_stats_total_licenses'] = 'רישיונות בחשבון';
$string['zoom_stats_storage_used'] = 'נפח אחסון בשימוש (הקלטות ענן)';
$string['zoom_stats_storage_gb'] = '{$a->gb} GB';
$string['zoom_stats_storage_na'] = 'לא זמין';
$string['zoom_revoke_inactive_license'] = 'הורדת רישיון ממשתמשים לא פעילים';
$string['zoom_revoke_inactive_license_desc'] = 'כאשר מופעל (רק Zoom), משימה מתוזמנת תהפוך משתמשים עם רישיון ל-Basic כאשר: (1) ההתחברות האחרונה הייתה לפני לפחות 6 שעות, (2) המשתמש לא נמצא כעת בפגישה, ו-(3) אין למשתמש פגישה מתוכננת ב-2 השעות הקרובות. משחרר רישיונות להקצאה מחדש. נדרשים scopes ב-Zoom: user:read:admin, user:write:admin; dashboard_meetings:read:admin נדרש לזיהוי משתמשים בפגישות חיות.';
$string['revoke_zoom_license_task'] = 'הורדת רישיון Zoom ממשתמשים לא פעילים';
$string['zoom_revoke_excluded_groups'] = 'החרגת קבוצות Zoom ממנגנון הורדת רישוי';
$string['zoom_revoke_excluded_groups_heading_desc'] = 'בחר קבוצות Zoom להחרגה מהמנגנון. משתמשים בקבוצות אלו לא יאבדו רישוי. <a href="{$a}">ניהול קבוצות מוחרגות</a>';
$string['zoom_revoke_excluded_groups_desc'] = 'בחר קבוצות Zoom שהמשתמשים בהן יוחרגו ממנגנון הורדת רישוי למשתמשים לא פעילים.';
$string['zoom_revoke_excluded_groups_error'] = 'לא ניתן לטעון קבוצות Zoom. יש לוודא שליישום Zoom יש הרשאות מתאימות (group:read:admin).';
$string['zoom_auto_license_teachers_first_login'] = 'מתן רישיון למורים בכניסה ראשונה';
$string['zoom_auto_license_teachers_first_login_desc'] = 'רק Zoom: כאשר משתמש עם תפקיד מורה או מורה עורך נכנס בפעם הראשונה, התוסף בודק את המשתמש ב-Zoom לפי אימייל ואם אין לו רישוי — מקצה רישוי (Licensed). רץ פעם אחת בלבד לכל משתמש.';
$string['zoom_auto_license_roles'] = 'תפקידים לזכאות לרישוי אוטומטי';
$string['zoom_auto_license_roles_desc'] = 'בחר אילו תפקידי Moodle יקבלו רישוי Zoom אוטומטית בכניסה ראשונה (כאשר מופעל).';
$string['aicreatequestionsmenu'] = 'יצירת שאלות מתוך הקלטות השיעור';
$string['questionfromvideopickvideo'] = 'בחרו הקלטת שיעור עם כתוביות (מוצגות רק כאלה). עשר הקלטות בעמוד.';
$string['questionfromvideosearchheading'] = 'חיפוש הקלטות';
$string['questionfromvideosearchlabel'] = 'חיפוש';
$string['questionfromvideosearchplaceholder'] = 'חיפוש לפי כותרת או מזהה…';
$string['questionfromvideosearchsubmit'] = 'חיפוש';
$string['questionfromvideosearchclear'] = 'נקה מסנן';
$string['questionfromvideosearchactive'] = 'מסנן פעיל: "{$a->q}"';
$string['questionfromvideosearchnoresults'] = 'לא נמצאו הקלטות התואמות לחיפוש.';
$string['questionfromstreammissingconfig'] = 'כתובת Stream או מפתח API לא הוגדרו באתר.';
$string['questionfromstreamnoemail'] = 'בפרופיל המשתמש אין כתובת דוא״ל; לא ניתן לטעון סרטונים מ-Stream.';
$string['questionfromstreaminvalidresponse'] = 'תשובה לא תקינה התקבלה מ-Stream. נסו שוב מאוחר יותר.';
$string['questionfromstreamnovideos'] = 'לא נמצאו סרטונים עם כתוביות עבור המשתמש שלך.';
$string['questionfromstreampageinfo'] = 'מוצגים {$a->from}–{$a->to} מתוך {$a->total}';
$string['questionfromstreamuntitled'] = '(ללא כותרת)';
$string['questionfromstreamapierror'] = 'לא ניתן לטעון סרטונים מ-Stream: {$a}';
$string['questionfromvideosectiongenerate'] = 'יצירת שאלות מתוך כתוביות';
$string['questionfromvideoquestioncount'] = 'מספר שאלות ליצירה';
$string['questionfromvideoqtypelabel'] = 'סוג שאלה';
$string['questionfromstreamgenerate'] = 'יצירת שאלות';
$string['questionfromstreamgeneratesuccess'] = 'השאלות נוצרו. יש לעיין בתוצאות למטה.';
$string['questionfromstreamgeneratefailed'] = 'לא ניתן ליצור שאלות. נסו שוב או בדקו את הסרטון והגדרות Stream.';
$string['questionfromstreampickvideo'] = 'יש לבחור סרטון מהרשימה.';
$string['questionfromstreampickvideorequired'] = 'חובה לבחור הקלטת שיעור מהרשימה למעלה (לחיצה על התצוגה המקדימה) לפני יצירת השאלות.';
$string['questionfromstreamformvideohint'] = 'קודם יש לבחור הקלטה מהרשימה למעלה (לחיצה על התצוגה המקדימה), ואז להגדיר את האפשרויות למטה.';
$string['questionfromstreaminvalidvideoid'] = 'נבחר סרטון לא תקין.';
$string['questionfromstreambloomlabel'] = 'רמת בלום';
$string['questionfromstreambankcategoryname'] = '{$a->title} [מזהה סרטון {$a->videoid}]';
$string['questionfromstreamnocatcap'] = 'נדרשת הרשאה לניהול קטגוריות בבנק השאלות בקורס כדי לשמור שאלות שנוצרו.';
$string['questionfromstreamsavefailed'] = 'לא ניתן לשמור את השאלות בבנק השאלות{$a}.';
$string['questionfromstreamimportnone'] = 'לא הוחזרו שאלות לשמירה.';
$string['questionfromstreamimporttype'] = 'השאלות שנוצרו לא תואמות לסוג שנבחר. נסו ליצור שוב.';
$string['questionfromvideoqtype_multichoice'] = 'רב-ברירה';
$string['questionfromvideoqtype_shortanswer'] = 'תשובה קצרה';
$string['questionfromvideoqtype_truefalse'] = 'נכון / לא נכון';
$string['qgenreporttitle'] = 'דוח שימוש — יצירת שאלות מהקלטות';
$string['qgenreportdatefrom'] = 'מתאריך';
$string['qgenreportdateto'] = 'עד תאריך';
$string['qgenreportfilter'] = 'הצג דוח';
$string['qgenreportperiod'] = 'תקופה: {$a->from} – {$a->to}';
$string['qgenreportsummary'] = 'סיכום';
$string['qgenreportmetric'] = 'מדד';
$string['qgenreportvalue'] = 'ערך';
$string['qgenreportcourses'] = 'קורסים';
$string['qgenreportquestions'] = 'שאלות שנוצרו';
$string['qgenreportgenerations'] = 'מחזורי יצירה';
$string['qgenreportusers'] = 'משתמשים';
$string['qgenreportbytype'] = 'לפי סוג שאלה';
$string['qgenreportbycourse'] = 'לפי קורס';
$string['qgenreportnodata'] = 'אין נתונים לתקופה שנבחרה.';
$string['qgenreportcoursedeleted'] = 'קורס שנמחק (#{$a})';
